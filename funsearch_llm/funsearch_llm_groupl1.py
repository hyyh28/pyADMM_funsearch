...
def evaluate(instances: dict) -> float:
    ...
    B = A @ X
    g_num = instances['g_num']
    g_len = round(na / g_num)
    G = [list(range(i * g_len, (i + 1) * g_len)) for i in range(g_num - 1)]
    G.append(list(range((g_num - 1) * g_len, na)))
    
    # Options for the group l1 minimization
    opts = instances
    # Perform group l1 minimization
    X2, obj, err, iter = groupl1_admm(A, B, G, opts)
...
if __name__ == '__main__':
    ...
    admm_groupl1_config = admm_utils.datasets['groupl1']
    global_max_sample_num = 100
    # if it is set to None, funsearch will execute an endless loop
    funsearch.main(
        specification=specification,
        inputs=admm_groupl1_config,
        config=config,
        max_sample_nums=global_max_sample_num,
        class_config=class_config,
        log_dir='logs/sparse_models/groupl1/funsearch_groupl1_100',
        temperature=0
    )
